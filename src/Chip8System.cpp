...
void Chip8System::cls(Instruction const instruction) noexcept
{
  std::ranges::fill(display, 0x0);
}
...
void Chip8System::lores(Instruction const instruction) noexcept
{
  ...
  current_width = Chip8System::LORES_WIDTH;
  current_height = Chip8System::LORES_HEIGHT;
...
void Chip8System::hires(Instruction const instruction) noexcept
{
  ...
  current_width = Chip8System::HIRES_WIDTH;
  current_height = Chip8System::HIRES_HEIGHT;
  display.resize(static_cast<size_t>(current_width * current_height));
}
...
void Chip8System::or_vx_vy(Instruction const instruction) noexcept
{
  ...
  // quirk
  registers.at(Chip8System::FLAG_REGISTER_IDX) = 0;
}
...
{
  const std::uint8_t register_x{ registers.at(instruction.x()) };
  const std::uint8_t register_y{ registers.at(instruction.y()) };

  registers.at(instruction.x()) = register_x + register_y;

  // Set flag register to the carry value
  registers.at(Chip8System::FLAG_REGISTER_IDX) = (register_x > 255 - register_y) ? 1 : 0;
}
...
void Chip8System::sub_vx_vy(Instruction const instruction) noexcept
{
  ...
  registers.at(Chip8System::FLAG_REGISTER_IDX) = (register_x >= register_y) ? 1 : 0;
}
void Chip8System::shr_vx_vy(Instruction const instruction) noexcept
{
  ...
  registers.at(Chip8System::FLAG_REGISTER_IDX) = (register_x & 0b00000001);
}
...
void Chip8System::rsb_vx_vy(Instruction const instruction) noexcept
{
  ...
  // Set flag register to the borrow value
  registers.at(Chip8System::FLAG_REGISTER_IDX) = (register_y >= register_x) ? 1 : 0;
}
...
void Chip8System::shl_vx_vy(Instruction const instruction) noexcept
{
  ...
  registers.at(instruction.x()) <<= 1; // Perform shift
  registers.at(Chip8System::FLAG_REGISTER_IDX) = register_x >> 7; // Set flag register to the bit shifted out
}
...
{
  const std::uint8_t register_x{ static_cast<std::uint8_t>(registers.at(instruction.x()) % current_width) };
  const std::uint8_t register_y{ static_cast<std::uint8_t>(registers.at(instruction.y()) % current_height) };
  ...
  for (std::uint8_t row = 0; row < n_row && register_y + row < current_height; row++)
  {
    for (std::uint8_t col = 0; col < n_col && register_x + col < current_width; col++)
    {
      ...
      if (pixel != 0)
      {
        const std::uint16_t idx{ static_cast<std::uint16_t>((register_y + row) * current_width + (register_x + col)) };
        if (display.at(idx) == 0x01)
        {
          registers.at(Chip8System::FLAG_REGISTER_IDX) = 1;
        }
        display.at(idx) ^= 1;
      }
    }
